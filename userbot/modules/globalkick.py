...
async def get_user_from_event(event):
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        if user.isnumeric():
            user = int(user)
        if not user:
            await event.edit(f"`{ALIVE_NAME}`: **Berikan nama jamet yang ingin kamu usir!!**")
            return
...
async def gspide(rk):
    ...
    if user:
        if user.id == 1282429349:
            return await rkp.edit(f"`{ALIVE_NAME}`, __Anda Tidak Bisa Global Kick Kepada Owner Saya Yang Tamvan__")
        try:
        ...
        await rkp.edit(f"`{ALIVE_NAME}:` **Balas ke jamet yang mau lo usir !! **")
