...
async def lastname(steal):
    ...
    if not steal.reply_to_msg_id:
        await steal.edit("```Mohon Balas Ke Pesan Jamet Yang Mau Lo Kepoin.```")
        return
    ...
    if message.sender.bot:
        await steal.edit("```ITU MAH BOT TOLOL.```")
        return
    ...
    try:
        async with bot.conversation(chat) as conv:
            ...
            if response.text.startswith("No records") or r.text.startswith(
                ...
                await steal.edit("```SI JAMET BELUM PERNAH GANTI NAMA.```")
                await steal.client.delete_messages(
...
CMD_HELP.update({
    "sangmata":
        "`.sa`\
          \nUsage: NIH KETEMU HISTORI NAMA JAMET."
})
