...
async def rm_deletedacc(show):
    ...
    del_u = 0
    del_status = "`Tidak Menemukan Akun Jamet, Grup Bersih`"

    if con != "clean":
        await show.edit("`Mencari Akun Jamet/Terhapus/Zombie...`")
        ...
        if del_u > 0:
            del_status = (
                f"`Menemukan` **{del_u}** `Akun Jamet/Terhapus/Zombie Dalam Grup Ini,"
    ...
    await show.edit("`Menghapus Akun Jamet...\nMohon Menunggu Sedang Dalam Proses`")
    del_u = 0
    del_a = 0
    ...
    if del_u > 0:
        del_status = f"`Membersihkan` **{del_u}** `Akun Jamet`"

    if del_a > 0:
        del_status = (
            f"Membersihkan **{del_u}** Akun Jamet "
            f"\n**{del_a}** `Admin Akun Jamet Tidak Bisa Dihapus.`"
        )
...
async def get_usersdel(show):
    ...
    try:
        ...
        await show.edit(
            "Grup Ini Terlalu Besar, Mengunggah Daftar Akun Jamet Sebagai File."
        )
        file = open("daftarpengguna.txt", "w+")
        file.write(mentions)
        file.close()
...
CMD_HELP.update(
    {
        ...
        "\n\n𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `.zombies`"
        "\n↳ : Untuk mencari Akun Jamet dalam grup."
        "Gunakan `.zombies clean` untuk menghapus Akun Jamet dari grup."
        "\n\n𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `.all`"
